...
enum class Msr : unsigned int {
  kIa32ApicBase = 0x01B,				// cpu的LAPAIC基址
  ...
  kIa32VmxBasic = 0x480,				//對於VMX的支持
  kIa32VmxPinbasedCtls = 0x481,			//用於?問如何?置Pin-based Vm-execution字段的1, 2, 4 位
  kIa32VmxProcBasedCtls = 0x482,		//用於?問如何?置Processor-based Vm-execution字段的1, 4-6, 8, 13-16, 26位
  kIa32VmxExitCtls = 0x483,
  kIa32VmxEntryCtls = 0x484,
  kIa32VmxMisc = 0x485,
  kIa32VmxCr0Fixed0 = 0x486,			//把CR0修正為0的位 
  kIa32VmxCr0Fixed1 = 0x487,			//把CR0修正為1的位
  kIa32VmxCr4Fixed0 = 0x488,			//同上
  kIa32VmxCr4Fixed1 = 0x489,			//同上
  kIa32VmxVmcsEnum = 0x48A,
  kIa32VmxProcBasedCtls2 = 0x48B,
  kIa32VmxEptVpidCap = 0x48C,			//VPID,EPT的能力報告
  kIa32VmxTruePinbasedCtls = 0x48D,		//用於?問如何?置Pin-based Vm-execution字段的其他保留位
  kIa32VmxTrueProcBasedCtls = 0x48E,    //用於?問如何?置Processor-based Vm-execution字段的其他保留位 -> bit[31]決定是否?置第二層Vm-Execution (肯定為1吧 , 因為要打?EPT)
  kIa32VmxTrueExitCtls = 0x48F,			//MSR相?
  kIa32VmxTrueEntryCtls = 0x490,		//MSR相?
...
enum class VmcsField : unsigned __int32 {
  ...
  kVmEntryIntrInfoField = 0x00004016,			//??位VMENTRY後表示要注入的事件 屬性vol3b 21-25
  kVmEntryExceptionErrorCode = 0x00004018,		//??位?置異常事件的異常碼 比如
...
struct VmControlStructure {
  unsigned long revision_identifier;	//使用哪一?版本維?vmcs , BIT[31] 表示是否shadow VMCS, 如為1 VMPTRLD指令失敗 通?MSR IA32_VMX_BASIC?取
  unsigned long vmx_abort_indicator;	//如果VMX中斷, ??CPU(某核心)會向他寫入
